...
public class MediaToneClient : ProxyClient
{
    ...
    protected override Task OnMessageAsync(MessageDirection direction, uint messageId, RfpIdentifier rfp, Memory<byte> data, CancellationToken cancellationToken)
    {
        ...
        var hdl = BinaryPrimitives.ReadUInt16LittleEndian(data.Span[4..]);
        if (data.Span[1] == 2)
        {
            if (data.Span[0] == 0)
            {
    ...
    private void ReadMidiFiles(CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        _audio.Add(("**##",
            new[]
            {
                new MediaToneMessage.Tone(34, 34, 34, 34, 19000, 17000, 20000, 18000, ushort.MaxValue, 0, 0, 0)
            }));
    }
    ...
    private void ReadMidiFile(string name, string number)
    {
        ...
        tones = tones
            .Concat(new[]
            {
                new MediaToneMessage.Tone(34, 34, 34, 34, 0, 0, 0, 0, ushort.MaxValue, 0, 0,
                    (ushort) (tones.Length + 1)),
                new MediaToneMessage.Tone(34, 34, 34, 34, 0, 0, 0, 0, ushort.MaxValue, 0, 0, (ushort) tones.Length)
            })
            .ToArray();
