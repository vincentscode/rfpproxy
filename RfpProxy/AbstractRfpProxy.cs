...
public abstract class AbstractRfpProxy : TcpProxy<CryptedRfpConnection>
{
        ...
        if (type.Span[0] == 0x01)
        {
            ...
            else if (type.Span[1] == 0x25)
            {
                ...
                using (var sha = SHA256.Create())
                {
                    ...
                    sysAuthenticate[4..].CopyTo(data);
                    pwBytes.CopyTo(data[0x20..]);
                    crypted.CopyTo(data[(0x20 + 0x1a)..]);
                ...
                sysAuthenticate[4..].CopyTo(aesKey);
        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                do
                {
                    ...
                    iv = block[^8..].ToArray();
                    await messageCallback(connection, plain[..(length + 4)], cancellationToken).ConfigureAwait(false);
    ...
    protected virtual void SetKeys(ReadOnlyMemory<byte> key, CryptedRfpConnection connection)
    {
        ...
        connection.SetOmmKey(key.Span[..56]);
        connection.SetRfpKey(key.Span[8..]);
    }
