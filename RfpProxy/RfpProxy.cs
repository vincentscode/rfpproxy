...
public class RfpProxy(int listenPort, string ommHost, int ommPort, string socket, string ommConf)
    : TransparentRfpProxy(listenPort, ommHost, ommPort)
{
    private readonly ConcurrentDictionary<Subscription, Subscription> _subscriptions = new();
    private readonly ConcurrentDictionary<RfpIdentifier, RfpConnection> _connections = new();
    ...
    {
        using var socket1 = new Socket(AddressFamily.Unix, SocketType.Stream, ProtocolType.IP)
        {
            ReceiveTimeout = 10,
            SendTimeout = 10,
        };
        ...
        socket1.Bind(new UnixDomainSocketEndPoint(socket));
        socket1.Listen(5);
        try
        {
            var accept = socket1.AcceptAsync(cancellationToken).AsTask();
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (accept.IsCompleted)
                {
                    ...
                    tasks.Remove(accept);
                    accept = socket1.AcceptAsync(cancellationToken).AsTask();
                    tasks.Add(accept);
                }
                else
                {
    ...
    {
        using var reader = new OmmConfReader(ommConf);
