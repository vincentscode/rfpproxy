...
class AaMiDePcapClient : PcapClient
{
    ...
    protected override void WritePacketHeader(byte[] header, MessageDirection direction, uint messageId, RfpIdentifier rfp, ReadOnlyMemory<byte> data)
    {
        ...
        span[1] = 0x10;//tos
        BinaryPrimitives.WriteUInt16BigEndian(span[2..], (ushort) (span.Length + data.Length)); //total length
        BinaryPrimitives.WriteUInt16BigEndian(span[4..], (ushort) messageId); //identification
        BinaryPrimitives.WriteUInt16BigEndian(span[6..], 0x4000); // flags
        span[8] = 0xff; //ttl
        span[9] = 6; //protocol
        if (direction == MessageDirection.ToOmm)
        {
            rfp.CopyTo(span[10..]); //source address
            span[12] = 127;
        ...
        {
            rfp.CopyTo(span[14..]); //destination address
            span[16] = 127;
        ...
        if (direction == MessageDirection.ToOmm)
        {
            ...
            BinaryPrimitives.WriteUInt16BigEndian(span[2..], 16321); //destination port
        }
        else
        {
            ...
            BinaryPrimitives.WriteUInt16BigEndian(span[2..], 54321); //destination port

        }
        BinaryPrimitives.WriteInt32BigEndian(span[4..], seq - data.Length);
        BinaryPrimitives.WriteInt32BigEndian(span[8..], ack);
