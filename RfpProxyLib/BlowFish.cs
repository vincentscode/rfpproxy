...
namespace RfpProxyLib
{
    public class BlowFish
    {
        ...
        const int ROUNDS = 16; //standard is 16, to increase the number of rounds, _roundKey needs to be equal to the number of rounds. Use digits of PI.
        
        //SBLOCKS
        private readonly uint[] _sbox0;
        private readonly uint[] _sbox1;
        private readonly uint[] _sbox2;
        private readonly uint[] _sbox3;

        private readonly uint[] _roundKey;
        ...
        /// </summary>
        /// <param name="key">Cipher key as a byte array</param>
        public BlowFish(ReadOnlySpan<byte> key)
        {
            ...
            if (key.Length > 56)
                throw new Exception("Key too long. Max 56 bytes allowed.");
            if (key.Length < 4)
                throw new CryptographicException("Key too short. At least 4 bytes required");
            
            var j = 0;
            for (var i = 0; i < 18; i++)
            {
                uint d = (uint)(((key[j % key.Length] * 256 + key[(j + 1) % key.Length]) * 256 + key[(j + 2) % key.Length]) * 256 + key[(j + 3) % key.Length]);
                _roundKey[i] ^= d;
                j = (j + 4) % key.Length;
            }
        ...
        /// </summary>
        /// <param name="iv">IV or last cipher block</param>
        /// <param name="data">plain/ciphertext</param>
        /// <param name="decrypt">true to decrypt, false to encrypt</param>
        /// <returns>(En/De)crypted data</returns>
        private byte[] Crypt_CBC(ReadOnlySpan<byte> iv, ReadOnlySpan<byte> data, bool decrypt)
        {
            ...
            if (!decrypt)
            {
                ...
                {
                    Buffer.BlockCopy(result, i, block, 0, 8);
                    XorBlock(block, localIv);
                    BlockEncrypt(block);
                    Buffer.BlockCopy(block, 0, localIv, 0, 8);
                    Buffer.BlockCopy(block, 0, result, i, 8);
                }
            }
            else
            {
                ...
                for (int i = 0; i < result.Length; i += 8)
                {
                    ...
                    BlockDecrypt(current);
                    XorBlock(current, localIv);
                    localIv = data.Slice(i, 8);
                }
            }
            return result;
        }
        ...
        public static void XorBlock(Span<byte> block, ReadOnlySpan<byte> iv)
        {
            ...
            BinaryPrimitives.WriteUInt64LittleEndian(block, result);
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Encipher(ref uint xlPar, ref uint xrPar)
        {
            xlPar ^= _roundKey[0];
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Decipher(ref uint xlPar, ref uint xrPar)
        {
            xlPar ^= _roundKey[17];
        ...
        private uint Round(uint a, uint b, uint n)
        {
            ...
            var b2 = (byte) (b >> 8);
            var b3 = (byte) b;
            uint x1 = (_sbox0[b0] + _sbox1[b1]) ^ _sbox2[b2];
            uint x2 = x1 + _sbox3[b3];
            uint x3 = x2 ^ _roundKey[n];
            return x3 ^ a;
        }
